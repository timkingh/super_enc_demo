...
#define IMG_DBG_FUNCTION             (0x00000001)
#define IMG_DBG_INFO                 (0x00000002)
#define IMG_DBG_TIME                 (0x00000004)

#define img_log(cond, fmt, ...)   do { if (cond) mpp_log_f(fmt, ## __VA_ARGS__); } while (0)
#define img_dbg(flag, fmt, ...)   img_log((img_debug & flag), fmt, ## __VA_ARGS__)
#define img_dbg_func(fmt, ...)    img_dbg(POST_DBG_FUNCTION, fmt, ## __VA_ARGS__)
#define img_dbg_info(fmt, ...)    img_dbg(IMG_DBG_INFO, fmt, ## __VA_ARGS__)
#define img_dbg_time(fmt, ...)    img_dbg(IMG_DBG_TIME, fmt, ## __VA_ARGS__)

static int img_debug = 0;
...
static int read_image_jpeg(const char* path, image_buffer_t* image)
{
    ...
    // 错误码为0时，表示警告，错误码丄1�71ￄ1�77-1时表示错评1�71ￄ1�77
...
static int read_image_yuv(const char* path, image_buffer_t* image)
{
    FILE *fp = fopen(path, "rb");
    if(fp == NULL) {
        printf("fopen %s fail!\n", path);
    ...
    image->width = 1920;
    image->height = 1080;
    // printf("file_size = %d %d\n", file_size, image->width * image->height * 3 / 2);
...
int read_image(const char* path, image_buffer_t* image)
{
    ...
    } else if (strcmp(_ext, ".yuv") == 0 || strcmp(_ext, ".YUV") == 0) {
        return read_image_yuv(path, image);
    }
    else {
                             ...
                             image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    time_start = mpp_time();
    if (src_img->use_dma32_buf) {
        rga_handle_src = importbuffer_fd(src_img->fd, &in_param);
        if (rga_handle_src == 0) {
            mpp_err_f("import src dma_fd error!\n");
            ret = -1;
            goto err;
        }
    ...
    if (dst_img->use_dma32_buf) {
        rga_handle_dst = importbuffer_fd(dst_img->fd, &dst_param);
        if (rga_handle_dst == 0) {
            mpp_err_f("import dst dma_fd error!\n");
            ret = -1;
            goto err;
        }
    ...
    time_end = mpp_time();
    img_dbg_time("RGA importbuffer time: %lld us\n", time_end - time_start);
    ...
    if (drect.width != dstWidth || drect.height != dstHeight) {
        ...
        p_imcolor[2] = color;
        p_imcolor[3] = color;
        img_dbg_info("fill dst image (x y w h)=(%d %d %d %d) with color=0x%x\n",
        ...
        time_end = mpp_time();
        img_dbg_time("RGA imfill time: %lld us\n", time_end - time_start);
    }
    ...
    if (ret_rga <= 0) {
        mpp_err_f("Error on improcess STATUS=%d\n", ret_rga);
        mpp_err_f("RGA error message: %s\n", imStrError((IM_STATUS)ret_rga));
        ret = -1;
    }
    time_end = mpp_time();
    img_dbg_time("RGA improcess time: %lld us\n", time_end - time_start);
...
err:
    ...
    time_end = mpp_time();
    img_dbg_time("RGA releasebuffer time: %lld us\n", time_end - time_start);
                                 ...
                                 letterbox_t* letterbox, char color)
{
    ...
    img_dbg_info("scale %f dst_box(%d %d %d %d) allow_slight_change %d "
    ...
    if(letterbox != NULL){
        ...
        // mpp_log_f("pad(%d, %d) scale %f\n", _left_offset, _top_offset, scale);
    }
